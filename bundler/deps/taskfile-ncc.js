...
module.exports = function (task) {
  ...
  task.plugin("ncc", {}, function* (file, nccOptions) {
    const { postAssets, postCode, ...options } = nccOptions
    ...
    return ncc(join(__dirname, file.dir, file.base), {
      ...
      .then(({ code, assets, ...rest }) => {
        Object.keys(assets).forEach((key) => {
          const base = basename(key)
          const dir = join(file.dir, dirname(key))

          const data =
            typeof postAssets === "function"
              ? postAssets(console.log, dir, base, assets[key]).source
              : assets[key].source
        ...
        if (options && options.packageName) {
          writePackageManifest.call(this, options.packageName, file.base, options.bundleName, precompiled)
        }

        file.data =
          typeof postCode === "function"
            ? Buffer.from(postCode(console.log, file, code), "utf8")
...
function writePackageManifest(packageName, main, bundleName, precompiled) {
  ...
  const compiledPackagePath = join(__dirname, `${!precompiled ? "dist/" : ""}compiled/${bundleName || packageName}`)
